# Rightclick section
execute at @p[scores={Right_Click=1..},nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",tag:{CustomModelData:201101}}}] run execute as @e[type=wolf,sort=nearest,limit=1,tag=tamed,distance=..2] if data entity @s Owner run function taming_book:stats/stats_wolf
execute at @p[scores={Right_Click=1..},nbt={SelectedItem:{id:"minecraft:warped_fungus_on_a_stick",tag:{CustomModelData:201101}}}] run execute as @e[type=cat,sort=nearest,limit=1,tag=tamed,distance=..2] if data entity @s Owner run function taming_book:stats/stats_cat
scoreboard players set @p[scores={Right_Click=1..}] Right_Click 0

# Taming
execute as @e[tag=tamed] run data merge entity @s {CustomNameVisible:1b}
...
execute as @e[tag=tamed] if score @s Tamed_LVL matches 100.. run effect give @s minecraft:instant_health 20 1 true
